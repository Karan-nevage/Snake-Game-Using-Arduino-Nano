...
{
  oled.setCursor(x, y); // Set display position
  oled.println(data);
}
...
void screen(void)
{
  ...
  oled.setCursor(104, 12); // Set display position
  oled.println("LEVE");
  oled.setCursor(104, 40); // Set display position
  oled.println("SCOR");
...
void draw_food(void)
{
  int food_out = 0; // Check if food is inside the snake
  ...
  if (food_eaten)
  {
    while (food_out == 0)
    {
      ...
      for (int i = snake_len - 1; i > 0; i--) // Traverse the entire snake body, if food is inside the snake then regenerate
      {
...
void snake_move(void)
{
  ...
  {
    food_eaten = true; // Can regenerate food
    snake_len++;
    score++;
    level = score / 5 + 1;
    snake_speed -= level;
...
void draw_game_over()
{
  ...
  oled.setCursor(10, 10); // Set display position

  oled.println("GAME OVER");
  oled.setTextSize(1); // Set font size
  oled.setCursor(30, 35); // Set display position
  oled.println("LEVE:");
  oled.setCursor(30, 55); // Set display position
  oled.println("SCOR:");
